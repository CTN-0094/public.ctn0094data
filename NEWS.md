# public.ctn0094 v1.1.1 (patch release)

## Enhancements
* Clarified `randomization$when` documentation description. Closes issue [#8](https://github.com/CTN-0094/public.ctn0094data/issues/8)

# public.ctn0094 v1.1.0 (CRAN release)

## Breaking  
* The `site_masked` variable has been updated to be 6 characters in length.

## Miscellaneous  
* Added labels to all tibbles

# public.ctn0094 v1.0.4

* Corrected the description of the fagerstrom table

# public.ctn0094 v1.0.3

* Fixed spelling errors
* Fixed URL re-directs
* Removed duplicated "project" column from `first_survey` data
* Fixed LaTeX manual errors

# public.ctn0094 v1.0.2

* Sorted everybody data set by project then by who
* Sorted all the other data sets to by who then when

# public.ctn0094 v1.0.1

* Fixed 11 incorrect dates
* Removed "High Stakes Manuscript" vignettes. 

# public.ctn0094 v1.0.0

* First CRAN release 
